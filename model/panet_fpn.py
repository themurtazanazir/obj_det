...
class PANetFPN(nn.Module):
    def __init__(self, in_channels_list, out_channels):
        ...
        for in_channels in in_channels_list:
            ...
            nn.init.kaiming_normal_(
                lateral_conv.weight, mode="fan_out", nonlinearity="relu"
            )
            nn.init.kaiming_normal_(
                fpn_conv.weight, mode="fan_out", nonlinearity="relu"
            )
    ...
    def forward(self, features):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            # Refine features
            laterals[i - 1] = self.top_down_blocks[used_backbone_levels - i - 1](
                laterals[i - 1]
            )
        ...
        for i in range(used_backbone_levels - 1):
            ...
            resized_lateral = F.interpolate(
                laterals[i + 1], size=downsample.shape[-2:], mode="nearest"
            )
